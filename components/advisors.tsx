          ...
          {advisors.map((a, i) => (
            <Reveal key={a.name} delay={i * 0.07}>
              <div className="card-base p-7 px-5 text-center relative overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-hover group">
                <span
                  aria-hidden
                  className="absolute top-[-50px] left-1/2 -translate-x-1/2 w-[200px] h-[140px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                  style={{
                ...
                {/* Avatar circle — use a.image (the current advisor) NOT advisors.map again */}
                <div
                  className="w-24 h-24 rounded-full mx-auto mb-4 overflow-hidden border-[3px] border-white relative z-10"
                  style={{
                ...
                <div className="meta text-slate pt-4 border-t border-navy/8 relative z-10">
                  <strong className="text-navy font-bold">{a.exp}</strong> &middot; {a.area}
                </div>
              </div>
            </Reveal>
          ))}
