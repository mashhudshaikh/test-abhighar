...
// Phase 4.5: Module-scope mutable bindings populated from props inside
// InteriorsPage so helper components defined OUTSIDE InteriorsPage
// (like Services, Packages, Portfolio) can still reference these names.
let PACKAGES: any[] = [];
let PORTFOLIO: any[] = [];
...
// Phase 4.5: Module-scope mutable bindings populated from server-fetched
// props inside InteriorsPage. These let helper components defined outside
// the main function (like Services) keep working without prop-drilling.
// Map service slug → icon name (fallback when API doesn't supply one)
function iconForSlug(slug: string | undefined): string {
  if (!slug) return "home";
  if (slug.includes("kitchen"))    return "kitchen";
  if (slug.includes("living"))     return "sofa";
  if (slug.includes("bedroom"))    return "bed";
  if (slug.includes("wardrobe"))   return "package";
  if (slug.includes("ceiling"))    return "package";
  if (slug.includes("bathroom"))   return "shield";
  if (slug.includes("foyer"))      return "home";
  if (slug.includes("study"))      return "book";
  if (slug.includes("kids"))       return "smile";
  if (slug.includes("puja"))       return "flame";
  if (slug.includes("full-home"))  return "home";
  return "home";
}

// Phase 4.5 — Module-scope mutable bindings for ALL CMS-driven sections.
// Populated from pageContent props inside InteriorsPage so helper
// components (Hero, Services, Process, FAQ) can read them directly.
let SECTION_HERO: { heroTitle: string; heroSubtitle: string; heroImageUrl: string } = {
  heroTitle: "Beautiful interiors, thoughtfully done.",
  heroSubtitle: "In-house designers shaping your home around your family's daily life — not the other way around.",
  heroImageUrl: "",
};
let SECTION_SERVICES: any[] = [];
let SECTION_USPS: { icon: string; title: string; description: string }[] = [];
let SECTION_PROCESS: { step: number; title: string; description: string }[] = [];
let SECTION_FAQS: { question: string; answer: string }[] = [];
let SECTION_TRUST: { label: string; value: string; sublabel?: string }[] = [];
let SECTION_ABOUT: { aboutTitle: string; aboutBody: string } = { aboutTitle: "", aboutBody: "" };
let SECTION_CTA: { ctaTitle: string; ctaSubtitle: string } = {
  ctaTitle: "Ready to start your home?",
  ctaSubtitle: "Free consultation. No commitment.",
};

let interiorAdvisor: any = {
  initials: "SK",
  name: "Sarika",
  role: "Senior Interior Designer",
  rating: 4.9,
};
let interiorCategories: any[] = [];

// Phase 4.5: INTERIORS_ADVISOR_DATA built from defaults at module scope.
// Real advisor data comes from props inside the component (see interiorAdvisor const).
...
// Phase 4.5 — Props passed in from server component
type InteriorPackageProp = {
  id: string;
  slug: string;
  name: string;
  tagline: string;
...
type InteriorPortfolioProp = {
  ...
  location: string;
  heroImageUrl: string;
  beforeImages: { url: string; alt?: string }[];
  afterImages: { url: string; alt?: string }[];
  galleryImages: { url: string; alt?: string; caption?: string }[];
};
type InteriorPageContentProp = {
  heroTitle: string;
  heroSubtitle: string;
  heroImageUrl: string;
  services: any[];
  categories: { slug: string; name: string; icon: string }[];
  trust: { label: string; value: string; sublabel?: string }[];
  aboutTitle: string;
  aboutBody: string;
  usps: any[];
  processSteps: any[];
  ...
  ctaTitle: string;
  ctaSubtitle: string;
  advisor: { name: string; initials: string; role: string; rating: number | null };
};
type InteriorsPageClientProps = {
  packages: InteriorPackageProp[];
  portfolio: InteriorPortfolioProp[];
  designers: any[];
  pageContent: InteriorPageContentProp;
};

export default function InteriorsPage({ packages, portfolio, designers, pageContent }: InteriorsPageClientProps) {
  // Phase 4.5 — Use API data if available, fall back to local defaults
  PACKAGES = packages.length > 0
    ? packages.map((p) => ({
        slug: p.slug,
        name: p.name,
        tagline: p.tagline,
        price: p.basePriceDisplay,
        popular: p.popular,
  ...
  PORTFOLIO = portfolio.length > 0
    ? portfolio.map((p, i) => ({
        ...
        location: p.location,
        image: p.heroImageUrl,
        // Phase 4.5 — additional rich gallery available:
        beforeImages: p.beforeImages,
        afterImages: p.afterImages,
        galleryImages: p.galleryImages,
      })) as any
    : DEFAULT_PORTFOLIO;
  // Override advisor & categories from API (assign to module-scope so
  // helper components like Services that reference these directly also work)
  interiorAdvisor = pageContent.advisor;
  interiorCategories = pageContent.categories.length > 0 ? pageContent.categories : [];

  // Phase 4.5 — populate all CMS-driven sections
  SECTION_HERO = {
    heroTitle: pageContent.heroTitle || SECTION_HERO.heroTitle,
    heroSubtitle: pageContent.heroSubtitle || SECTION_HERO.heroSubtitle,
    heroImageUrl: pageContent.heroImageUrl || SECTION_HERO.heroImageUrl,
  };
  SECTION_SERVICES = pageContent.services || [];
  SECTION_USPS = pageContent.usps || [];
  SECTION_PROCESS = pageContent.processSteps && pageContent.processSteps.length > 0
    ? pageContent.processSteps
    : DEFAULT_PROCESS;
  SECTION_FAQS = pageContent.faqs && pageContent.faqs.length > 0
    ? pageContent.faqs
    : DEFAULT_FAQS;
  SECTION_TRUST = pageContent.trust || [];
  SECTION_ABOUT = {
    aboutTitle: pageContent.aboutTitle || "",
    aboutBody: pageContent.aboutBody || "",
  };
  (INTERIORS_ADVISOR_DATA as any).advisor = {
    initials: interiorAdvisor.initials || "SK",
    name: interiorAdvisor.name || "Sarika",
    role: interiorAdvisor.role || "Senior Interior Designer",
    rating: interiorAdvisor.rating ?? 4.9,
  };
  SECTION_CTA = {
    ctaTitle: pageContent.ctaTitle || SECTION_CTA.ctaTitle,
    ctaSubtitle: pageContent.ctaSubtitle || SECTION_CTA.ctaSubtitle,
  };
...
function Services() {
  // Phase 4.5 — read from API (SECTION_SERVICES populated in InteriorsPage)
  // with fallback to hardcoded list if API returns empty.
  const all = SECTION_SERVICES.length > 0
    ? SECTION_SERVICES.map((s: any, i: number) => ({
        slug: s.slug || `svc-${i}`,
        name: s.title || s.name || "Service",
        icon: s.icon || iconForSlug(s.slug),
      }))
    : [
          ...
          <Reveal key={s.step} delay={i * 0.08} className="h-full">
            <div className="card-base p-4 sm:p-6 h-full flex flex-row sm:flex-col items-start gap-3 sm:gap-0 transition-all duration-300 hover:-translate-y-1 hover:shadow-hover hover:border-gold/30 group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="meta text-slate text-[12px] sm:text-[13px] leading-relaxed">{s.description ?? s.desc}</p>
              </div>
            </div>
          </Reveal>
        ))}
            ...
            <Reveal key={i}>
              <details open={isOpen} onToggle={(e) => setOpen((e.target as HTMLDetailsElement).open ? i : null)} className="card-base bg-white px-3.5 sm:px-5 py-3 sm:py-4 group transition-all hover:border-gold/30">
                <summary className="list-none cursor-pointer flex items-center justify-between gap-2.5 sm:gap-4">
                  <span className="font-sans font-semibold text-[13px] sm:text-[15.5px] text-navy leading-snug">{f.question ?? f.q}</span>
                ...
                <p className="body-base text-slate mt-2.5 sm:mt-3 text-[12.5px] sm:text-[14px] leading-relaxed">{f.answer ?? f.a}</p>
              </details>
            </Reveal>
          );
        })}
